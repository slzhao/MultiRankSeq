...
##' MultiRankSeqReport(output="MultiRankSeq1.html",diffResult=exampleCountsDiff,rawCounts=exampleCounts)
##' \dontrun{
##' #An example: from counts data to report.
##' #this code may take two minutes, because performing baySeq, DESeq, and edgeR may be slow. 
##' MultiRankSeqReport(output="MultiRankSeq2.html",rawCounts=exampleCounts)
##' }
MultiRankSeqReport<-function(output="MultiRankSeq.html",rawCounts,group=c(0,0,0,1,1,1),diffResult,...) {
	if (missing(diffResult)) {
		diffResult<-findDiff(rawCounts=rawCounts,group=group)
	}
	diffReport(output=output,diffResult=diffResult,rawCounts=rawCounts,group=group,...)
}
...
findDiff<-function(rawCounts,group=c(0,0,0,1,1,1)) {
	comparDESeq<-myDESeq(rawCounts,group)
	comparEdgeR<-myedgeR(as.matrix(rawCounts),group)
	comparBayseq<-mybayseq(as.matrix(rawCounts),group)
	resultCombinedCompar<-combined_result(DESeq=comparDESeq,edgeR=comparEdgeR,bayseq=comparBayseq,rawCount=rawCounts)
...
#	bayseq<- bayseq[order(as.numeric(rownames(bayseq))),c("Likelihood","FDR.DE")]
	colnames(bayseq)<-c("Likelihood", "AdjLikelihood")
	return(bayseq)
}

myDESeq<-function(count, group){
#	require(DESeq)
	condition<-factor( group )
	y_DESeq<-newCountDataSet( count, condition )
	y_DESeq<-estimateSizeFactors( y_DESeq )
	y_DESeq<-estimateDispersions( y_DESeq, method="per-condition", fitType="local")
	DEseq<-nbinomTest( y_DESeq, "0", "1")
	return(DEseq)
}

myedgeR<-function(count, group){
#	require(edgeR)
	y<-DGEList(count, group=group )
	y <- calcNormFactors(y)
	y <- estimateCommonDisp(y, verbose=TRUE)
	y <- estimateTagwiseDisp(y)
	edgeR <- topTags( exactTest(y), dim(count)[1] )
	return(edgeR$table)
}

combined_result<-function(DESeq,edgeR,bayseq,rawCount,comparGroups=c(0,0,0,1,1,1),rankMethod1=1) {
	...
	temp1<-temp1[temp]
	compar1Result<-cbind(DESeq[temp,c(6,7,8)],edgeR[temp,c(1,3,4)],rawLogFold=temp1,OneMLikelihood=(1-bayseq[temp,1]),AdjLikelihood=bayseq[temp,2])
	...
	colnames(result)[1:12]<-c("log2FoldChange(DESeq)","pValue(DESeq)","pAdj(DESeq)","log2FoldChange(edgeR)","pValue(edgeR)","pAdj(edgeR)","log2FoldChange(raw)","1-Likelihood(baySeq)","AdjLikelihood(baySeq)","rank(DESeq)","rank(edgeR)","rank(baySeq)")
	return(result)
}
...
panel.cor <- function(x, y, digits=2, prefix="", cex.cor) 
{
	...
	par(usr = c(0, 1, 0, 1)) 
	r <- abs(cor(x, y,use="pa"))
	txt <- format(c(r, 0.123456789), digits=digits)[1] 
	txt <- paste(prefix, txt, sep="") 
	if(missing(cex.cor)) cex.cor <- 0.8/strwidth(txt) 
	
	test <- cor.test(x,y,use="pa") 
...
#	require(VennDiagram)
	...
	} else if (method=="p") { #p cutoff
		for (x in 1:3) {
			vennResult[[x]]<-row.names(result)[which(result[,x*3]<=cutoffP)]
			titleText<-paste("Cutoff: p value <=",cutoffP,sep="")
		}
	...
	names(vennResult)<-c("DESeq","edgeR","baySeq")
