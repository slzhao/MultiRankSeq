...
##' }
MultiRankSeqReport<-function(output="MultiRankSeq.html",rawCounts,group=c(0,0,0,1,1,1),diffResult,seed=NULL,paired=NULL,exportCounts=F,useVStCount=T,...) {
	...
	diffReport(output=output,diffResult=diffResult,rawCounts=rawCounts,group=group,paired=paired,exportCounts=exportCounts,useVStCount=useVStCount,...)
	return(diffResult)
}
...
diffReport<-function(input=system.file("extdata", "report.Rhtml", package = "MultiRankSeq"),output,diffResult,rawCounts,group=c(0,0,0,1,1,1),paired=NULL,exportCounts=F,useVStCount=T) {
#	require(knitr)
#	require(VennDiagram)
...
combined_result2<-function(DESeq,edgeR,bayseq,rawCount,comparGroups=c(0,0,0,1,1,1),rankMethod1=1) {
	...
	temp1<-temp1[temp]
	compar1Result<-cbind(DESeq[temp,c(2,5,6)],edgeR[temp,c(1,4,5)],rawLogFold=temp1,OneMLikelihood=(1-bayseq[temp,1]),AdjLikelihood=bayseq[temp,2])
